...
const NotFound = async () => {
  // Get the current locale from headers since not-found.tsx doesn't receive params
  const headersList = await headers();
  const pathname = headersList.get('x-pathname') || '';
  const referer = headersList.get('referer') || '';
  
  // Try to detect language from pathname or referer
  let lang = 'en';
  if (pathname.startsWith('/es') || referer.includes('/es')) {
    lang = 'es';
  }
            ...
            <RevealAnimation delay={0.1}>
              <div className="bg-background-3 dark:bg-background-5 rounded-4xl border-[10px] border-white dark:border-background-9 relative overflow-hidden flex flex-col justify-center items-center py-10 md:py-20 lg:py-[100px] text-center pr-2.5">
                ...
                <RevealAnimation delay={0.6} instant>
                  <div>
                    <LinkButton href={`/${lang}`} className="btn btn-lg btn-primary hover:btn-secondary dark:hover:btn-accent">
